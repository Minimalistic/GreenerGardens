...
export function lookupZoneClient(latitude: number): string | null {
  for (const band of ZONE_BANDS) {
    if (latitude >= band.minLat && latitude < band.maxLat) return band.zone;
  }
...
export function lookupFrostDatesClient(zone: string) {
