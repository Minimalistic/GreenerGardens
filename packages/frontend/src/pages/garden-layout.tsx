...
import type { SubPlot, ApiResponse, PlotType, SunExposure, IrrigationType, Plot } from '@gardenvault/shared';
...
export function GardenLayout() {
  ...
  const handlePaste = useCallback(async () => {
    ...
    try {
      ...
      await createPlot.mutateAsync({
        ...
        sun_exposure: clipboard.sun_exposure ?? undefined,
        irrigation: clipboard.irrigation ?? undefined,
  ...
  const handleDuplicate = useCallback(async () => {
    ...
    try {
      const newPlot = await createPlot.mutateAsync({
        ...
        sun_exposure: plot.sun_exposure ?? undefined,
        irrigation: plot.irrigation ?? undefined,
  ...
  const handlePlotDragEnd = useCallback(async (plotId: string, geometry: { x: number; y: number; width: number; height: number; rotation: number }) => {
    try {
      ...
      const data: Record<string, unknown> = { geometry };
      if (sizeChanged) {
