...
export class TimelineService {
  ...
  getEvents(options: {
    ...
    ).all(...params, limit, offset) as { field_changes_json: string | null; snapshot_json: string | null; id: string; entity_type: string; entity_id: string; action: string; timestamp: string; changed_by: string; notes: string | null }[];
  ...
  getMarkers(options: {
    ...
    const rows = this.db.prepare(`
      ...
      ORDER BY date ASC
    `).all(options.start, options.end) as { date: string; count: number; entity_type: string; type_count: number }[];
