...
export class AnalyticsService {
  ...
  getYieldByPlant(year?: number) {
    const yearFilter = year ? `AND strftime('%Y', h.date_harvested) = ?` : '';
    const params = year ? [String(year)] : [];
    return this.db.prepare(`
      SELECT
        pc.common_name as plant_name,
        pi.variety_name as variety,
      ...
      GROUP BY pc.common_name, pi.variety_name, h.unit
  ...
  getYieldByPlot(year?: number) {
    ...
    return this.db.prepare(`
      ...
      WHERE 1=1 ${yearFilter}
      GROUP BY p.name, h.unit
      ORDER BY total_quantity DESC
    `).all(...params);
  }

  getSeasonalTimeline(year?: number) {
    const yearFilter = year ? `WHERE strftime('%Y', h.date_harvested) = ?` : '';
    const params = year ? [String(year)] : [];
    return this.db.prepare(`
      SELECT
        strftime('%Y-%m', h.date_harvested) as month,
        COUNT(*) as harvest_count,
        SUM(h.quantity) as total_quantity,
        h.unit
      FROM harvests h
      ${yearFilter}
      GROUP BY month, h.unit
      ORDER BY month ASC
    `).all(...params);
  }
  ...
  getYearOverYear() {
    return this.db.prepare(`
      SELECT
        strftime('%Y', h.date_harvested) as year,
        COUNT(*) as harvest_count,
        SUM(h.quantity) as total_quantity
      FROM harvests h
      GROUP BY year
