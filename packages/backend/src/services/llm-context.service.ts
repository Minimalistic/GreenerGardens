...
export class LlmContextService {
  ...
  buildContext(gardenId: string): string {
    ...
    ).all(gardenId) as { id: string; name: string; plot_type: string | null; sun_exposure: string | null }[];
    if (plots.length > 0) {
      sections.push(`\n## Plots (${plots.length})`);
    ...
    `).all(gardenId) as { status: string; date_planted: string | null; health: string; notes: string | null; common_name: string; plant_type: string; days_to_maturity_min: number | null; plot_name: string }[];
    if (plants.length > 0) {
    ...
    const tasks = this.db.prepare(`
      ...
      LIMIT 10
    `).all() as { title: string; task_type: string; due_date: string | null; priority: string }[];
    if (tasks.length > 0) {
      sections.push(`\n## Pending Tasks`);
